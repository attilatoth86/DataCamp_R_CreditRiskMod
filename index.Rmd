...
We had been informed about the content of the columns in the course but without that the columns were very intuitively named.
...
```{r simple plots, fig.align='center', cache=TRUE}
par(mfrow=c(1,2))
...
Luckily, our dataset was more or less tidy therefore no observation occured below the lower boundaries that would have actually made sense. Outer the upper boundaries we could see significant number of observations. The thresholds set for age and annual income variables, using the general outlier threshold rule, are sort of strict, given that we might assume by nature that customers above 40 years and $140k still belonged appropriately to our dataset. Therefore I decided to remove only those two extreme outliers (annual income above $6M and age above 130) from the dataset. (This happened to be one single observation.)
...
```{r adj histograms, fig.align='center', cache=TRUE, fig.height=15}
par(mfrow=c(4,2))
...
In case of missing data, the following strategies can be considered:
 ...
 - **Deletion**: to delete observation or the entire variable. If there are only few observation having missing values, observations are to be deleted; in case of large number of missing value in a column, the deletion of the variable should be considered.
...
loan_data_nonOL_nonNA <- loan_data_nonOL[-NA_idx_del,]
```
...
set.seed(123)
train_idx <- sample(1:nrow(loan_data_nonOL_nonNA),2/3*nrow(loan_data_nonOL_nonNA))

loan_data_train <- loan_data_nonOL_nonNA[train_idx, ]
loan_data_test <- loan_data_nonOL_nonNA[-train_idx, ]
```
