...
Similar in many ways to linear regression except for that the output value of the model is between 0 and 1. That's why logistic regression models can be good for probability estimation as these values fall by definition between 0 and 1. 

P(loan_status=1 | X1,X2,...,Xm) = 1 / (1 + e^-[B0+B1X1+B2X2+...+BmXm])

[B0+B1X1+B2X2+...+BmXm] - linear predictor; combination of parameters and variables

To fit logistic regression model, we can use the generalized linear model function, `glm()`. To order R for fitting logistic model, we have to set `family = "binomial"`.
...
Intercept is the estimate for B0, values under various variables are estimates for B1, B2,..., etc.

Probability of default divided by probability of non-default gives the odds in favor of default that equals the exponential function of the linear predictor.

Interpretation of coefficients: if a value Xj goes up by 1 -> the odds are multiplied by e^Bj.

 - Odds decrease as Xj increases: Bj<0 -> e^Bj<1
 - Odds increase as Xj decreases: Bj>0 -> e^Bj>1
 
In our model it means, if age goes up by 1 the odds for default are multiplied by e^-0.007576 ~ 0.9925. One extra age lowers the probability of default by nearly 1%.
